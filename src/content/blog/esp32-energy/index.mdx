 ...
 * the `VCC` port of the PPK2 to the `5V0` [^2] pin of the ESP32 device, and
...
<figure>
  ...
  <figcaption>
    When the ESP32-H2 runs the [ESP32 OpenThread boilerplate deep sleep SED program](https://github.com/espressif/esp-idf/tree/master/examples/openthread/ot_sleepy_device/deep_sleep) and has its `5V0` pin connected to the `VCC` port of the PPK2, the current measured by the PPK2 is around ~1.21 mA, rather than ~7 uA.
  </figcaption>
</figure>
...
* Connect the `VCC` port of the PPK2 to the `3V3`[^6] pin of the ESP32-H2, or to one of the two J5 pins that are visible when the J5 jumper has been removed.
