...
You set up an ESP32 with the PPK2 by using jumper wires to connect:

 * the `VCC` port of the PPK2 to the `5V0` [^1] pin of the ESP32 device, and
 * the `GND` port of the PPK2 to `GND` pin on the ESP32.

In this configuration, you can use the PPK2 to measure the current of the ESP32 device while the PPK2 itself is providing a 5V of power. This is how I initially approached the measurement of my ESP32-H2 device.

However, this approach will *not* work when the ESP32 is consuming really low currents in the uA range: the PPK2 will report much higher currents than expected.

For instance, when measuring the deep sleep current of my ESP32-H2 when running the [ESP32 OpenThread boilerplate deep sleep SED program](https://github.com/espressif/esp-idf/tree/master/examples/openthread/ot_sleepy_device/deep_sleep), the PPK2 reports a current of ~1.25 mA range when it should be measuring currents within the ~7-8 uA range.
