...
I took a two-step approach in order to remove the use of AES at the MAC layer. First, I removed the functionality for doing MAC encryption. To confirm that I have successfully removed encryption, I made sure that I could see the application layer payloads of packets sent by my ESP32s in plaintext in Wireshark. Only after that I have confirmed that I have successfully removed encryption, I began working on removing the functionality for decryption. 

When I have successfully removed the use of both AES encryption and decryption at the MAC layer, I was able to see the application layer payloads in plaintext, and the devices were able to receive such data without any issues.
...
Although MLE packet payloads are above the physical and data link layers in the Thread network stack, they are nonetheless are still encrypted under AES-CCM, following 802.15.4-2006 specification. Immediately before an MLE packet is transmitted, the payload gets encrypted using a symmetric key that is distinct[^4] from the one used to encrypt MAC layer payloads. 

The creation of the nonce is exactly the same as done for MAC layer packets, following the nonce format described in 802.15.4-2006. A four byte MIC gets appended to the end of the MLE payload for the purposes of data integrity. The Thread Specification state that the default size is 32 bits for all MICs generated under both MLE and MAC encryption.

Decryption is handled in the same manner as the decryption of MAC frames. The nonce is recreated from the packet header, and the payload gets decrypted. In addition, the MIC corresponding to the ciphertext gets recreated, and is compared to the MIC in the MLE payload. If the MICs match, the ciphertext is valid and the payload is sent for further processing. Otherwise, the MLE packet gets dropped.

AES-CCM encryption and decryption for packets sent under MLE are handled separately from MAC frames. The encryption and decryption of MLE packets are handled in software, and the implementation
for both operations are defined in the `src/thread/mle.cpp` source file of the Espressif OpenThread fork. Furthermore, both MLE encryption and decryption are handled in a single method: `Mle::ProcessMessageSecurity()`, which takes as an argument an enumeration of type `Crypto::AesCcm::Mode` to determine whether the caller wants to encrypt or decrypt. The `Mle::ProcessMessageSecurity()` method is only ever directly used in `mle.cpp` and by two other methods:

* The `Mle::TxMessage::SendTo()` method to encrypt outbound packets that are to be transmitted.
* The `Mle::HandleUdpReceive()` method to decrypt inbound packets that have just been received.

As a result, I just had to remove the respective calls to `Mle::ProcessMessageSecurity()` in these two functions in order to remove both AES-CCM encryption and decryption for all MLE packets.

The first step I took was to remove just encryption, by removing only removing the call to `Mle::ProcessMessageSecurity()` in `Mle::TxMessage::SendTo()`. After doing so, I was able to see that Wireshark was *not* able to decrypt the MLE packets sent by my ESP32s, as shown in the figure below.

This allowed me to infer that I have successfully removed AES-CCM encryption for MLE payloads. However, since I only removed MLE encryption, and not decryption, the ESP32s were not able to form Thread network with each other. As a result, they all formed separate partitions that contained only themselves as the respective network leaders.

After I have confirmed that I have successfully removed AES-CCM encryption for MLE packets, I then removed decryption by removing the call to `Mle::ProcessMessageSecurity()` in `Mle::HandleUdpReceive()`. After doing so, my ESP32s were able to form Thread networks together.

<figure>
  <img src="/images/thesis-blog-post/mle-plaintext.png" width="800" /> 
  <figcaption>
  The fact that Wireshark was unable to decrypt the MLE packets, even when my devices can still communicate with each others proves that I have successfully removed MLE encryption and decryption.
  </figcaption>
</figure>

## Adding ASCON AEAD into OpenThread

For each place in which in the act of doing AES encryption and decryption was removed, it was replaced with the respective ASCON encryption and decryption functions. In this section, I will describe how I added the [LibAscon](https://github.com/TheMatjaz/LibAscon) implementation of ASCON-128a and ASCON-128 into the modified plaintext source code of OpenThread.

Before I begin my discussion of how I integrated [LibAscon](https://github.com/TheMatjaz/LibAscon) into OpenThread, I need to note that the LibAscon implementation of ASCON AEAD is used, rather than the official ASCON C implementations of ASCON-128a and ASCON-128, as it enables to the use of these two ciphers with a tag length that is different that the size of 16 bytes that is used in the official ASCON specification. This particular feature of LibAscon was particularly helpful, as it allowed me to create ASCON ciphertext that have tag which were 4 bytes in length, the default size of MICs for MAC and MLE ciphertexts, as stated in the Thread specification. 


### LibAscon Wrapper Functions
...
[^4]: According to the Thread specification, two different symmetric keys are separately for the purposes of MAC and MLE encryption.