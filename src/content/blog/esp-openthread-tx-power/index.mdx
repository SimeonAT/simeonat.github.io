...
Most settings on ESP32 devices would typically be set using `Kconfig` variables and configured using `idf.py menuconfig`. However, there is no such `Kconfig` configuration for the TX power.
...
In addition, [my ESP32 programming projects](https://github.com/UCSC-ThreadAscon) contain examples on how to use the OpenThread TX power class methods. One such example is in my project where I implement an ESP32-H2 Sleepy End Device (SED). In this project, I [defined wrapper funtions](https://github.com/UCSC-ThreadAscon/energy-usage-sed-simple/blob/main/main/txpower.c) on top of the `otPlatRadio` TX power functions, and I use those functions to [set and query the TX power](https://github.com/UCSC-ThreadAscon/energy-usage-sed-simple/blob/main/main/main.c#L253) of the SED.